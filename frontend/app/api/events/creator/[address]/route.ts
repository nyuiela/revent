...
export async function GET(
  ...
  try {
    ...
    console.log(`Fetching events by creator: ${address} from The Graph Protocol...`);

    // Define the query to find events by creator address
    const query = `{
      eventCreateds(where: { creator: "${address}" }, orderBy: blockTimestamp, orderDirection: desc) {
        id
        eventId
        creator
        ipfsHash
    ...
    console.log(`Found ${graphEvents.length} events for creator ${address}`);
    ...
    console.log(`Successfully processed ${validEvents.length} events for creator ${address}`);
    ...
    console.error("Error fetching events by creator from The Graph Protocol:", error);
