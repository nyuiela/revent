...
export default function EventPage({ eventId, ipfsHash, idType }: Props) {
  const [isScrolling, setIsScrolling] = useState(false);
  const [lastScrollY, setLastScrollY] = useState(0);
  const [ticketMode, setTicketMode] = useState<"none" | "single" | "multiple">("single");
  ...
  // Fetch event metadata from IPFS - either from direct ipfsHash or from contract eventData
  useEffect(() => {
    let aborted = false;
    async function run() {
      let ipfsUri = ipfsHash;

      // If we have eventData from contract but no direct ipfsHash, use the contract's IPFS URI
  ...
  const startISO: string | undefined = ipfsData?.startISO;
  const endISO: string | undefined = ipfsData?.endISO;
  ...
  return (
    <div className="min-h-screen text-[var(--events-foreground)] bg-black/80 relative z-[20]">
      ...
      <div className="p-0 max-w-7xl mx-auto space-y-0 pt-6 mt-4 bg-red-transparent">
        <EventManagement eventId={ipfsHash as string} defaultIpfsHash="bafkreia2uchzmzosieaj6tyim4qzq5xvxhlyapq2gzacen2tffknfeco6u" />
            ...
            <div className="flex items-center gap-3">
              {canTransact ? (
                <Transaction
                  ...
                  onStatus={(s: any) => setIsRegistering(s.statusName === "transactionPending" || s.statusName === "buildingTransaction")}
                >
            ...
            <ParticipantsGrid
              addresses={
                Array.isArray(attendeesData)
                  ? attendeesData.map((att: any) => att?.attendee || att?.wallet || att).filter(Boolean)
                  : []
              }
              maxItems={4}
            />
