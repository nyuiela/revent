export default function SponsorsSection() {
  ...
  return (
    <div className="bg-[#FF8C00] py-16 px-8">
        ...
        <div className="bg-white rounded-lg shadow-lg p-8">
          <div className="grid grid-cols-3 gap-8">
            {sponsors.map((sponsor, index) => (
              <div key={index} className={`text-center ${sponsor.special ? 'bg-orange-100 p-6 rounded-lg' : 'p-4'} relative`}>
                {/* Logo */}
                <div className={`w-16 h-16 ${getLogoColor(sponsor.color)} rounded-full mx-auto mb-4 flex items-center justify-center`}>
                  <span className="text-white text-2xl font-bold">{sponsor.logo}</span>
                </div>

                {/* Special Badge for Cardtonic */}
                {sponsor.badge && (
                  <div className="bg-white text-black px-3 py-1 rounded text-xs font-bold mb-2 inline-block">
                    {sponsor.badge}
                  </div>
                )}
                ...
                {sponsor.hasButton && (
                  <div className="absolute -bottom-2 -right-2">
                    <button className="bg-yellow-400 hover:bg-yellow-500 text-black font-bold py-2 px-4 rounded-lg shadow-lg flex items-center space-x-2 transition-colors">
        ...
        <div className="mt-12 text-center">
          <div className="bg-white rounded-lg shadow-lg p-8 max-w-4xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button className="bg-[#FF8C00] hover:bg-[#E67E00] text-white font-bold py-3 px-6 rounded-lg transition-colors">
