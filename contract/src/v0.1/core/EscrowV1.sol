...
contract EscrowV1 is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, IEscrowV1 {
    ...
    modifier onlyTicketsContract() {
        require(
            _msgSender() == ticketsContract || _msgSender() == factoryContract,
            "Only tickets contract or factory can call this"
        );
        _;
    }
     ...
     */
    function createEscrow(uint256 eventId) external onlyEventCreator(eventId) nonReentrant whenNotPaused {
        ...
        whenNotPaused
    {
        ...
        require(
            totalRefundedAmount[eventId] + eventDeposits[eventId][_msgSender()].amount <= escrow.depositedAmount,
            "Total refunded amount exceeds deposited amount"
        );
        escrow.updatedAt = block.timestamp;
