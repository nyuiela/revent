...
//  */
contract StreamEvents is
    EventAttendees,
    EventQueries,
    EventAdmin,
    EventTickets,
    EventDomaIntegration,
    EventTrading,
    ERC2771Context
{
    ...
    function _generateConfirmationCode(
        uint256 eventId,
        address attendee
    )
        internal
    ...
    function approveOwnershipTokenOperator(
        address operator,
        bool approved
    ) external onlyOwner {
    ...
    function getUSDC() external view returns (address) {
        return marketplaceUSDC;
    }

    function getWETH() external view returns (address) {
        return marketplaceWETH;
    }
    ...
    function getProtocolFee()
        external
        view
        returns (address receiver, uint256 feeBps)
    {
    ...
    function setOrderExpirationTime(
        ...
        emit EventEvents.OrderExpirationTimeUpdated(
            oldExpiration,
            newExpirationTime
        );
    }
    ...
    function setDefaultInvestorApprovalRequired(
        uint256 eventId,
        bool required,
        ...
        emit EventEvents.InvestorApprovalRequired(
            eventId,
            required,
            thresholdBps
        );
